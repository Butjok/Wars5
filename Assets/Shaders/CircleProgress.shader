Shader "Unlit/CircleProgress"
{
 ...
 _Offset ("_Offset", Float) = 1
        ...
        _ProgressSmoothness ("_ProgressSmoothness", Float) = 0.1
        
        _Head ("_Head", Range(0, 1)) = 0
        _HeadSmoothness ("_HeadSmoothness", Float) = 0.1
        ...
        SubShader
    {
        ...
        CGPROGRAM
            ...
            fixed _SpeedX;
            fixed _SpeedY;
            fixed _Thickness, _Size, _Smoothness, _Offset, _Progress, _ProgressSmoothness, _Head, _HeadSmoothness;
 		...
 		OUT.texcoord1.xy = v.texcoord1.xy;
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                color.rgb = IN.color;
                float arc =atan2(-(IN.texcoord.x - .5),-(IN.texcoord.y - .5)) / 6.28318530718 + .5;
                color.a = min(color.a, smoothstep(_Progress - _ProgressSmoothness, _Progress + _ProgressSmoothness, arc));

                float head = smoothstep(_Head-_HeadSmoothness,_Head+_HeadSmoothness, arc);
                color.rgb += head * _Color;
