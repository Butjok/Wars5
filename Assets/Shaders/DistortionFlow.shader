Shader "Custom/DistortionFlow" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
            ...
            fixed3 normal3 = UnpackNormal( tex2D (_Normal, TRANSFORM_TEX(((position/1.5)/8 + float2(_Time.x*.125/2, _Time.x*.125/2)),_Normal)));
            fixed3 normal = UnpackNormal( tex2D (_Normal, TRANSFORM_TEX(((position/1.5)*2 + float2(_Time.x*3/2, 0)),_Normal)));
            fixed3 normal2 = UnpackNormal( tex2D (_Normal, TRANSFORM_TEX(((position/1.5) - float2(0, _Time.x*4.676/2)),_Normal)));
