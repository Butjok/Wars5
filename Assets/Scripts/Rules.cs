...
public static class Rules {
	...
	public static int? Damage(Unit attacker, Unit target, int weaponIndex) {
		if (Ammo(attacker, weaponIndex) <= 0 || Damage(attacker.type, target.type, weaponIndex) is not { } baseDamage)
			return null;
		return CeilToInt((float)attacker.hp.v / MaxHp(attacker) * baseDamage);
	}
	...
	public static bool CanAttack(Unit attacker, Unit target, int weaponIndex) {
		...
		return CanAttack(attacker.type, target.type, weaponIndex) &&
		       ...
		       attacker.position.v is { } attackerPosition &&
		       target.position.v is { } targetPosition &&
		       MathUtils.ManhattanDistance(attackerPosition, targetPosition).IsIn(AttackRange(attacker));
	}
	public static bool CanAttack(Unit attacker,Unit target) {
		return Weapons(attacker).Any(weaponIndex => CanAttack(attacker, target, weaponIndex));
	}
	public static bool IsArtillery(UnitType unitType) {
		return (UnitType.Artillery & unitType) != 0;
	}
	public static bool CanRespondToAttack(UnitType unitType) {
		return !IsArtillery(unitType);
	}
	public static bool CanRespondToAttack(Unit attacker, Unit target) {
		return CanRespondToAttack(target.type) &&
		       CanAttack(target, attacker);
	}
	...
	public static bool CanStay(Unit unit, Vector2Int position) {
		return unit.level.TryGetTile(position, out var tile) &&
		       CanStay(unit, tile) &&
	...
	public static bool CanJoin(Unit unit, Unit other) {
		return unit.player == other.player && other.hp.v < MaxHp(other);
	}
}