	...
	public static IEnumerator New(Game2 game, Unit unit, Vector2Int startForward, MovePath path) {
		...
		while (true) {
			...
			if (game.CurrentPlayer.IsAi || Input.GetKeyDown(KeyCode.Return) && actions.Count>0) {
				...
				if (Rules.Won(game.realPlayer)) {
					var (controlFlow, nextState) = game.levelLogic.OnVictory(game);
				...
				else if (Rules.Lost(game.realPlayer)) {
					var (controlFlow, nextState) = game.levelLogic.OnDefeat(game);
					yield return nextState;
					if (controlFlow == ControlFlow.Replace)
						yield break;
