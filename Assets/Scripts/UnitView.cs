...
public class UnitView : MonoBehaviour {
	...
	private void UpdateSprings(bool isCalledFromUpdate) {
		if (lastSpringUpdateTime is { } actualLastTime) {
			...
			if (TryCalculateAcceleration(isCalledFromUpdate && drawAccelerationGraph, out _, out var acceleration)) {
				var torque = (float)(acceleration * accelerationTorqueMultiplier);
				if (torque > maxTorque || torque < -maxTorque)
					torque = Mathf.Sign(torque) * maxTorque;
				accelerationTorque = -body.right * torque;
			}
			else
